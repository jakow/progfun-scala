...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase().groupBy(_.toChar).toList.map({ case (a, b) => (a, b.length) }).sortBy(_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      for {
        repetitions <- 0 to head._2
        rest <- combinations(tail)
      } yield if (repetitions > 0) (head._1, repetitions) :: rest else rest
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if (y.isEmpty) x
      ...
      y.foldLeft(x.toMap)((curr: Map[Char, Int], elem: (Char, Int)) =>
        if (curr(elem._1) == elem._2) curr - elem._1 // remove key from the map
        else curr.updated(elem._1, curr(elem._1) - elem._2)
      )
    }.toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
          ...
          combination <- combinations(occurrences).filter(dictionaryByOccurrences.contains(_)) // only combinations in dictionary
